package com.fastcampus.job.pass;
...
public class AddPassesTaskletTest {
    ...
    public void test_execute(){
        ...
        // when
        when(bulkPassRepository.findByStatusAndStartedAtGreatedThan(eq(BulkPassStatus.READY), any())).thenReturn(List.of(bulkPassEntity))
        when(userGroupMappingRepository.findByUserGroupId(eq("GROUP"))).thenReturn(List.of(userGroupMappingEntity));

        RepeatStatus repeatStatus = addPassesTasklet
        ...
        assertEquals(packageSeq, passEntity.getPassSeq());
